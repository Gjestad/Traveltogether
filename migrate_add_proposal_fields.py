...
with app.app_context():
    ...
    try:
        db.session.execute(db.text("ALTER TABLE trip_proposal ADD COLUMN departure_location_is_final TINYINT(1) DEFAULT 0"))
        print("✓ Added departure_location_is_final column")
    except Exception as e:
        print(f"departure_location_is_final: {e}")

    try:
        db.session.execute(db.text("ALTER TABLE trip_proposal ADD COLUMN destination_is_final TINYINT(1) DEFAULT 0"))
